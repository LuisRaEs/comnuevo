...
export default function InputDate({id,label,controlled,placeHolder,inputWidth,style,regex, title,state,setState, disabled}) {
    ...
    const handleChange = (e)=>{
        setState(oldState=>{return{...oldState,[e.target.name]:new Date(e.target.value).toISOString() }})
    ...
    return(
        ...
        <Form.Control 
            ...
            value = {!chkDisabled?(state[id] == null?"":state[id].split("T")[0]):""}
            onChange ={ e=>{
                    handleChange(e)
                }
            }
